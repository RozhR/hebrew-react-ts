...
export function GrammarProvider({ children }: GrammarProviderProps) {
    const [words, setWords] = useState<GrammarWordRef[]>(loadGrammarWords);

    useEffect(() => {
        localStorage.setItem(STORAGE_KEY, JSON.stringify(words));
    }, [words]);

    const isInGrammar = (word: GrammarWordRef): boolean => {
        return words.some((item) => item.category === word.category && item.id === word.id);
    };

    const addWord = (word: GrammarWordRef) => {
    ...
    const removeWord = (word: GrammarWordRef) => {
        setWords((currentWords) =>
            currentWords.filter(
                (item) => !(item.category === word.category && item.id === word.id),
            ),
