...
function assertContinuousIds(rows, maxId, fileName) {
  ...
  if (ids.length !== maxId) {
    throw new Error(
      `${fileName}: expected ${maxId} rows, got ${ids.length}`,
    );
  }
  ...
  for (let id = 1; id <= maxId; id += 1) {
    if (ids[id - 1] !== id) {
      throw new Error(
        `${fileName}: expected id ${id}, got ${ids[id - 1]}`,
      );
    }
  }
}
...
function assertWordMatches({
  ...
  for (const row of rows) {
    ...
    if (!base) {
      throw new Error(
        `${fileName}: id ${row.id} does not exist in base table`,
      );
    }
...
if (data.verbsFutureImperative.length !== 500) {
  throw new Error(
    "verbs_future_imperative.xlsx: expected 500 rows",
  );
}
...
if (data.adjectivesConstructions.length !== 88) {
  throw new Error(
    "adjectives_constructions.xlsx: expected 88 rows",
  );
}
if (data.adjectivesExamples.length !== 500) {
  throw new Error(
    "adjectives_examples.xlsx: expected 500 rows",
  );
}
